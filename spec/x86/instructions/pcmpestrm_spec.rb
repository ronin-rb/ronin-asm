...
require 'spec_helper'
require 'ronin/asm/x86/instructions/pcmpestrm'

require_relative '../helpers/operands'

describe Ronin::ASM::X86::Instructions::PCMPESTRM do
  ...
  describe "#initialize" do
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: pcmpestrm #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :pcmpestrm" do
      expect(subject.name).to eq(:pcmpestrm)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "pcmpestrm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "pcmpestrm_xmm_mem128_imm8.bin" }
