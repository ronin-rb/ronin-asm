...
describe Ronin::ASM::X86::Instructions::AAD do
  ...
  describe "#initialize" do
    it "must set #name to :aad" do
      expect(subject.name).to be(:aad)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: aad #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :aad" do
      expect(subject.name).to eq(:aad)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types imm8" do
      ...
      let(:bin_file_name) { "aad_imm8.bin" }
