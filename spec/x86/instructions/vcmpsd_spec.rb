...
require 'spec_helper'
require 'ronin/asm/x86/instructions/vcmpsd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86::Instructions::VCMPSD do
  ...
  describe "#initialize" do
    ...
    context "when given operands of types k{k}, xmm, mem64, imm8" do
      let(:operands) { [k_k(0), xmm(1), mem64(2), imm8(3)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:"k{k}", :xmm, :mem64, :imm8]' do
        expect(subject.form).to eq([:"k{k}", :xmm, :mem64, :imm8])
      end
    end

    context "when given operands of types k, xmm, mem64, imm8" do
      let(:operands) { [k(0), xmm(1), mem64(2), imm8(3)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it "must set #form to [:k, :xmm, :mem64, :imm8]" do
        expect(subject.form).to eq([:k, :xmm, :mem64, :imm8])
      end
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vcmpsd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vcmpsd" do
      expect(subject.name).to eq(:vcmpsd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types k{k}, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vcmpsd_k{k}_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types k, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vcmpsd_k_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types k{k}, xmm, mem64, imm8" do
      let(:operands) { [k_k(0), xmm(1), mem64(2), imm8(3)] }

      let(:bin_file_name) { "vcmpsd_k{k}_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types k, xmm, mem64, imm8" do
      let(:operands) { [k(0), xmm(1), mem64(2), imm8(3)] }

      let(:bin_file_name) { "vcmpsd_k_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vcmpsd_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64, imm8" do
      let(:operands) { [xmm(0), xmm(1), mem64(2), imm8(3)] }

      let(:bin_file_name) { "vcmpsd_xmm_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types k{k}, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vcmpsd_k{k}_xmm_xmm_{sae}_imm8.bin" }
    ...
    context "when #operands contains operands of types k, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vcmpsd_k_xmm_xmm_{sae}_imm8.bin" }
