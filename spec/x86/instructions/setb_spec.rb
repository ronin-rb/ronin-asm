...
require 'spec_helper'
require 'ronin/asm/x86/instructions/setb'

require_relative '../helpers/operands'

describe Ronin::ASM::X86::Instructions::SETB do
  ...
  describe "#initialize" do
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: setb #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :setb" do
      expect(subject.name).to eq(:setb)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types reg8" do
      ...
      let(:bin_file_name) { "setb_reg8.bin" }
    ...
    context "when #operands contains operands of types mem8" do
      ...
      let(:bin_file_name) { "setb_mem8.bin" }
