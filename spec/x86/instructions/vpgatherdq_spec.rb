...
require 'spec_helper'
require 'ronin/asm/x86/instructions/vpgatherdq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86::Instructions::VPGATHERDQ do
  ...
  describe "#name" do
    it "must return :vpgatherdq" do
      expect(subject.name).to eq(:vpgatherdq)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}, vm32x" do
      ...
      let(:bin_file_name) { "vpgatherdq_xmm{k}_vm32x.bin" }
    ...
    context "when #operands contains operands of types ymm{k}, vm32x" do
      ...
      let(:bin_file_name) { "vpgatherdq_ymm{k}_vm32x.bin" }
    ...
    context "when #operands contains operands of types zmm{k}, vm32y" do
      ...
      let(:bin_file_name) { "vpgatherdq_zmm{k}_vm32y.bin" }
    ...
    context "when #operands contains operands of types xmm, vm32x, xmm" do
      ...
      let(:bin_file_name) { "vpgatherdq_xmm_vm32x_xmm.bin" }
    ...
    context "when #operands contains operands of types ymm, vm32x, ymm" do
      ...
      let(:bin_file_name) { "vpgatherdq_ymm_vm32x_ymm.bin" }
