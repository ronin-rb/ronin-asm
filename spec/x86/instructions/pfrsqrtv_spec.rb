...
require 'spec_helper'
require 'ronin/asm/x86/instructions/pfrsqrtv'

require_relative '../helpers/operands'

describe Ronin::ASM::X86::Instructions::PFRSQRTV do
  ...
  describe "#initialize" do
    it "must set #name to :pfrsqrtv" do
      expect(subject.name).to be(:pfrsqrtv)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: pfrsqrtv #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :pfrsqrtv" do
      expect(subject.name).to eq(:pfrsqrtv)
    end
  end
