...
require 'spec_helper'
require 'ronin/asm/x86/instructions/vaesdec'

require_relative '../helpers/operands'

describe Ronin::ASM::X86::Instructions::VAESDEC do
  ...
  describe "#initialize" do
    it "must set #name to :vaesdec" do
      expect(subject.name).to be(:vaesdec)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vaesdec #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vaesdec" do
      expect(subject.name).to eq(:vaesdec)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vaesdec_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vaesdec_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm" do
      ...
      let(:bin_file_name) { "vaesdec_ymm_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256" do
      ...
      let(:bin_file_name) { "vaesdec_ymm_ymm_mem256.bin" }
    ...
    context "when #operands contains operands of types zmm, zmm, zmm" do
      ...
      let(:bin_file_name) { "vaesdec_zmm_zmm_zmm.bin" }
    ...
    context "when #operands contains operands of types zmm, zmm, mem512" do
      ...
      let(:bin_file_name) { "vaesdec_zmm_zmm_mem512.bin" }
