...
require 'spec_helper'
require 'ronin/asm/x86/instructions/vmovdqu'

require_relative '../helpers/operands'

describe Ronin::ASM::X86::Instructions::VMOVDQU do
  ...
  describe "#initialize" do
    it "must set #name to :vmovdqu" do
      expect(subject.name).to be(:vmovdqu)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vmovdqu #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vmovdqu" do
      expect(subject.name).to eq(:vmovdqu)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "vmovdqu_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "vmovdqu_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm" do
      ...
      let(:bin_file_name) { "vmovdqu_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, mem256" do
      ...
      let(:bin_file_name) { "vmovdqu_ymm_mem256.bin" }
    ...
    context "when #operands contains operands of types mem128, xmm" do
      ...
      let(:bin_file_name) { "vmovdqu_mem128_xmm.bin" }
    ...
    context "when #operands contains operands of types mem256, ymm" do
      ...
      let(:bin_file_name) { "vmovdqu_mem256_ymm.bin" }
