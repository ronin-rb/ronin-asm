...
describe Ronin::ASM::X86::Instructions::RDPID do
  ...
  describe "#initialize" do
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: rdpid #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :rdpid" do
      expect(subject.name).to eq(:rdpid)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types reg32" do
      ...
      let(:bin_file_name) { "rdpid_reg32.bin" }
