...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/xchg'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::XCHG do
  ...
  describe "#initialize" do
    ...
    context "when given operands of types rax, reg64" do
      let(:operands) { [rax(0), reg64(1)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it "must set #form to [:rax, :reg64]" do
        expect(subject.form).to eq([:rax, :reg64])
      end
    end

    context "when given operands of types reg64, rax" do
      let(:operands) { [reg64(0), rax(1)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it "must set #form to [:reg64, :rax]" do
        expect(subject.form).to eq([:reg64, :rax])
      end
    end
  ...
  describe "#name" do
    it "must return :xchg" do
      expect(subject.name).to eq(:xchg)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types rax, reg64" do
      let(:operands) { [rax(0), reg64(1)] }

      let(:bin_file_name) { "xchg_rax_reg64.bin" }
    ...
    context "when #operands contains operands of types reg64, rax" do
      let(:operands) { [reg64(0), rax(1)] }

      let(:bin_file_name) { "xchg_reg64_rax.bin" }
    ...
    context "when #operands contains operands of types reg64, reg64" do
      ...
      let(:bin_file_name) { "xchg_reg64_reg64.bin" }
    ...
    context "when #operands contains operands of types reg64, mem64" do
      ...
      let(:bin_file_name) { "xchg_reg64_mem64.bin" }
    ...
    context "when #operands contains operands of types mem64, reg64" do
      ...
      let(:bin_file_name) { "xchg_mem64_reg64.bin" }
