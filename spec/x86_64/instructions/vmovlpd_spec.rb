...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vmovlpd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VMOVLPD do
  ...
  describe "#initialize" do
    it "must set #name to :vmovlpd" do
      expect(subject.name).to be(:vmovlpd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vmovlpd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vmovlpd" do
      expect(subject.name).to eq(:vmovlpd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mem64, xmm" do
      ...
      let(:bin_file_name) { "vmovlpd_mem64_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64" do
      ...
      let(:bin_file_name) { "vmovlpd_xmm_xmm_mem64.bin" }
