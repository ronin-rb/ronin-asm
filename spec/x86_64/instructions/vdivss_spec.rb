...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vdivss'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VDIVSS do
  ...
  describe "#initialize" do
    it "must set #name to :vdivss" do
      expect(subject.name).to be(:vdivss)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vdivss #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vdivss" do
      expect(subject.name).to eq(:vdivss)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vdivss_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem32" do
      ...
      let(:bin_file_name) { "vdivss_xmm{k}{z}_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vdivss_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem32" do
      ...
      let(:bin_file_name) { "vdivss_xmm_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vdivss_xmm{k}{z}_xmm_xmm_{er}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vdivss_xmm_xmm_xmm_{er}.bin" }
