...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/cvtps2pi'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::CVTPS2PI do
  ...
  describe "#initialize" do
    it "must set #name to :cvtps2pi" do
      expect(subject.name).to be(:cvtps2pi)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: cvtps2pi #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mmx, xmm" do
      ...
      let(:bin_file_name) { "cvtps2pi_mmx_xmm.bin" }
    ...
    context "when #operands contains operands of types mmx, mem64" do
      ...
      let(:bin_file_name) { "cvtps2pi_mmx_mem64.bin" }
