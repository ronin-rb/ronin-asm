...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/phsubw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::PHSUBW do
  ...
  describe "#initialize" do
    it "must set #name to :phsubw" do
      expect(subject.name).to be(:phsubw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: phsubw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :phsubw" do
      expect(subject.name).to eq(:phsubw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mmx, mmx" do
      ...
      let(:bin_file_name) { "phsubw_mmx_mmx.bin" }
    ...
    context "when #operands contains operands of types mmx, mem64" do
      ...
      let(:bin_file_name) { "phsubw_mmx_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "phsubw_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "phsubw_xmm_mem128.bin" }
