...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/cmpoxadd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::CMPOXADD do
  ...
  describe "#initialize" do
    it "must set #name to :cmpoxadd" do
      expect(subject.name).to be(:cmpoxadd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: cmpoxadd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :cmpoxadd" do
      expect(subject.name).to eq(:cmpoxadd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mem32, reg32, reg32" do
      ...
      let(:bin_file_name) { "cmpoxadd_mem32_reg32_reg32.bin" }
    ...
    context "when #operands contains operands of types mem64, reg64, reg64" do
      ...
      let(:bin_file_name) { "cmpoxadd_mem64_reg64_reg64.bin" }
