...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vinsertps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VINSERTPS do
  ...
  describe "#initialize" do
    it "must set #name to :vinsertps" do
      expect(subject.name).to be(:vinsertps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vinsertps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vinsertps" do
      expect(subject.name).to eq(:vinsertps)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vinsertps_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem32, imm8" do
      ...
      let(:bin_file_name) { "vinsertps_xmm_xmm_mem32_imm8.bin" }
