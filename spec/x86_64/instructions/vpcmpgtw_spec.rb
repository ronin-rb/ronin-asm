...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpcmpgtw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPCMPGTW do
  ...
  describe "#initialize" do
    it "must set #name to :vpcmpgtw" do
      expect(subject.name).to be(:vpcmpgtw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpcmpgtw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpcmpgtw" do
      expect(subject.name).to eq(:vpcmpgtw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types k{k}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_k{k}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types k, xmm, xmm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_k_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types k{k}, xmm, mem128" do
      let(:operands) { [k_k(0), xmm(1), mem128(2)] }

      let(:bin_file_name) { "vpcmpgtw_k{k}_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types k, xmm, mem128" do
      let(:operands) { [k(0), xmm(1), mem128(2)] }

      let(:bin_file_name) { "vpcmpgtw_k_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types k{k}, ymm, ymm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_k{k}_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types k, ymm, ymm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_k_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types k{k}, ymm, mem256" do
      let(:operands) { [k_k(0), ymm(1), mem256(2)] }

      let(:bin_file_name) { "vpcmpgtw_k{k}_ymm_mem256.bin" }
    ...
    context "when #operands contains operands of types k, ymm, mem256" do
      let(:operands) { [k(0), ymm(1), mem256(2)] }

      let(:bin_file_name) { "vpcmpgtw_k_ymm_mem256.bin" }
    ...
    context "when #operands contains operands of types k{k}, zmm, zmm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_k{k}_zmm_zmm.bin" }
    ...
    context "when #operands contains operands of types k, zmm, zmm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_k_zmm_zmm.bin" }
    ...
    context "when #operands contains operands of types k{k}, zmm, mem512" do
      let(:operands) { [k_k(0), zmm(1), mem512(2)] }

      let(:bin_file_name) { "vpcmpgtw_k{k}_zmm_mem512.bin" }
    ...
    context "when #operands contains operands of types k, zmm, mem512" do
      let(:operands) { [k(0), zmm(1), mem512(2)] }

      let(:bin_file_name) { "vpcmpgtw_k_zmm_mem512.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vpcmpgtw_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm" do
      ...
      let(:bin_file_name) { "vpcmpgtw_ymm_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256" do
      ...
      let(:bin_file_name) { "vpcmpgtw_ymm_ymm_mem256.bin" }
