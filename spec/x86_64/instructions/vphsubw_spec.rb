...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vphsubw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPHSUBW do
  ...
  describe "#initialize" do
    it "must set #name to :vphsubw" do
      expect(subject.name).to be(:vphsubw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vphsubw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vphsubw" do
      expect(subject.name).to eq(:vphsubw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vphsubw_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vphsubw_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm" do
      ...
      let(:bin_file_name) { "vphsubw_ymm_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256" do
      ...
      let(:bin_file_name) { "vphsubw_ymm_ymm_mem256.bin" }
