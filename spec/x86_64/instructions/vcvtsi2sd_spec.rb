...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vcvtsi2sd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VCVTSI2SD do
  ...
  describe "#initialize" do
    it "must set #name to :vcvtsi2sd" do
      expect(subject.name).to be(:vcvtsi2sd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vcvtsi2sd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vcvtsi2sd" do
      expect(subject.name).to eq(:vcvtsi2sd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, reg32" do
      ...
      let(:bin_file_name) { "vcvtsi2sd_xmm_xmm_reg32.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, reg64" do
      ...
      let(:bin_file_name) { "vcvtsi2sd_xmm_xmm_reg64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem32" do
      ...
      let(:bin_file_name) { "vcvtsi2sd_xmm_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64" do
      ...
      let(:bin_file_name) { "vcvtsi2sd_xmm_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, {er}, reg64" do
      ...
      let(:bin_file_name) { "vcvtsi2sd_xmm_xmm_{er}_reg64.bin" }
