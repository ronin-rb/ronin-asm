...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/jo'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::JO do
  ...
  describe "#initialize" do
    it "must set #name to :jo" do
      expect(subject.name).to be(:jo)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: jo #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types rel32" do
      ...
      let(:bin_file_name) { "jo_rel32.bin" }
