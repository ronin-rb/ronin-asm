...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpcomub'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPCOMUB do
  ...
  describe "#initialize" do
    it "must set #name to :vpcomub" do
      expect(subject.name).to be(:vpcomub)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpcomub #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpcomub" do
      expect(subject.name).to eq(:vpcomub)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vpcomub_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vpcomub_xmm_xmm_mem128_imm8.bin" }
