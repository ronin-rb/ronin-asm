...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpmadcswd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPMADCSWD do
  ...
  describe "#initialize" do
    it "must set #name to :vpmadcswd" do
      expect(subject.name).to be(:vpmadcswd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpmadcswd #{operands.map(&:type).join(', ')}")
      end
    end
