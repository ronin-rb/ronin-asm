...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfixupimmsd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFIXUPIMMSD do
  ...
  describe "#initialize" do
    it "must set #name to :vfixupimmsd" do
      expect(subject.name).to be(:vfixupimmsd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfixupimmsd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfixupimmsd" do
      expect(subject.name).to eq(:vfixupimmsd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vfixupimmsd_xmm{k}{z}_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem64, imm8" do
      let(:operands) { [xmm_k_z(0), xmm(1), mem64(2), imm8(3)] }

      let(:bin_file_name) { "vfixupimmsd_xmm{k}{z}_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vfixupimmsd_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64, imm8" do
      ...
      let(:bin_file_name) { "vfixupimmsd_xmm_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}, imm8" do
      let(:operands) { [xmm_k_z(0), xmm(1), xmm(2), sae(3), imm8(4)] }

      let(:bin_file_name) { "vfixupimmsd_xmm{k}{z}_xmm_xmm_{sae}_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}, imm8" do
      let(:operands) { [xmm(0), xmm(1), xmm(2), sae(3), imm8(4)] }

      let(:bin_file_name) { "vfixupimmsd_xmm_xmm_xmm_{sae}_imm8.bin" }
