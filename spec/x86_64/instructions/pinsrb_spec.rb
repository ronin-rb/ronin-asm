...
describe Ronin::ASM::X86_64::Instructions::PINSRB do
  ...
  describe "#name" do
    it "must return :pinsrb" do
      expect(subject.name).to eq(:pinsrb)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, reg32, imm8" do
      ...
      let(:bin_file_name) { "pinsrb_xmm_reg32_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem8, imm8" do
      let(:operands) { [xmm(0), mem8(1), imm8(2)] }

      let(:bin_file_name) { "pinsrb_xmm_mem8_imm8.bin" }
