...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/movupd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::MOVUPD do
  ...
  describe "#initialize" do
    it "must set #name to :movupd" do
      expect(subject.name).to be(:movupd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: movupd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :movupd" do
      expect(subject.name).to eq(:movupd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "movupd_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "movupd_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types mem128, xmm" do
      ...
      let(:bin_file_name) { "movupd_mem128_xmm.bin" }
