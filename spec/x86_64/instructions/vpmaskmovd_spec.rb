...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpmaskmovd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPMASKMOVD do
  ...
  describe "#initialize" do
    it "must set #name to :vpmaskmovd" do
      expect(subject.name).to be(:vpmaskmovd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpmaskmovd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpmaskmovd" do
      expect(subject.name).to eq(:vpmaskmovd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vpmaskmovd_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256" do
      ...
      let(:bin_file_name) { "vpmaskmovd_ymm_ymm_mem256.bin" }
    ...
    context "when #operands contains operands of types mem128, xmm, xmm" do
      ...
      let(:bin_file_name) { "vpmaskmovd_mem128_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types mem256, ymm, ymm" do
      ...
      let(:bin_file_name) { "vpmaskmovd_mem256_ymm_ymm.bin" }
