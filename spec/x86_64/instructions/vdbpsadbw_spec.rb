...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vdbpsadbw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VDBPSADBW do
  ...
  describe "#initialize" do
    it "must set #name to :vdbpsadbw" do
      expect(subject.name).to be(:vdbpsadbw)
    end
    ...
    context "when given operands of types xmm{k}{z}, xmm, mem128, imm8" do
      let(:operands) { [xmm_k_z(0), xmm(1), mem128(2), imm8(3)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:"xmm{k}{z}", :xmm, :mem128, :imm8]' do
        expect(subject.form).to eq([:"xmm{k}{z}", :xmm, :mem128, :imm8])
      end
    end
    ...
    context "when given operands of types ymm{k}{z}, ymm, mem256, imm8" do
      let(:operands) { [ymm_k_z(0), ymm(1), mem256(2), imm8(3)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:"ymm{k}{z}", :ymm, :mem256, :imm8]' do
        expect(subject.form).to eq([:"ymm{k}{z}", :ymm, :mem256, :imm8])
      end
    end
    ...
    context "when given operands of types zmm{k}{z}, zmm, mem512, imm8" do
      let(:operands) { [zmm_k_z(0), zmm(1), mem512(2), imm8(3)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:"zmm{k}{z}", :zmm, :mem512, :imm8]' do
        expect(subject.form).to eq([:"zmm{k}{z}", :zmm, :mem512, :imm8])
      end
    end
    ...
    context "when given operands of types zmm, zmm, mem512, imm8" do
      ...
      it "must set #form to [:zmm, :zmm, :mem512, :imm8]" do
        expect(subject.form).to eq([:zmm, :zmm, :mem512, :imm8])
      end
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vdbpsadbw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vdbpsadbw" do
      expect(subject.name).to eq(:vdbpsadbw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_xmm{k}{z}_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem128, imm8" do
      let(:operands) { [xmm_k_z(0), xmm(1), mem128(2), imm8(3)] }

      let(:bin_file_name) { "vdbpsadbw_xmm{k}{z}_xmm_mem128_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm{k}{z}, ymm, ymm, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_ymm{k}{z}_ymm_ymm_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm{k}{z}, ymm, mem256, imm8" do
      let(:operands) { [ymm_k_z(0), ymm(1), mem256(2), imm8(3)] }

      let(:bin_file_name) { "vdbpsadbw_ymm{k}{z}_ymm_mem256_imm8.bin" }
    ...
    context "when #operands contains operands of types zmm{k}{z}, zmm, zmm, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_zmm{k}{z}_zmm_zmm_imm8.bin" }
    ...
    context "when #operands contains operands of types zmm{k}{z}, zmm, mem512, imm8" do
      let(:operands) { [zmm_k_z(0), zmm(1), mem512(2), imm8(3)] }

      let(:bin_file_name) { "vdbpsadbw_zmm{k}{z}_zmm_mem512_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_xmm_xmm_mem128_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_ymm_ymm_ymm_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_ymm_ymm_mem256_imm8.bin" }
    ...
    context "when #operands contains operands of types zmm, zmm, zmm, imm8" do
      ...
      let(:bin_file_name) { "vdbpsadbw_zmm_zmm_zmm_imm8.bin" }
    ...
    context "when #operands contains operands of types zmm, zmm, mem512, imm8" do
      let(:operands) { [zmm(0), zmm(1), mem512(2), imm8(3)] }

      let(:bin_file_name) { "vdbpsadbw_zmm_zmm_mem512_imm8.bin" }
