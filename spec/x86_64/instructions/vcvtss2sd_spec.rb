...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vcvtss2sd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VCVTSS2SD do
  ...
  describe "#initialize" do
    it "must set #name to :vcvtss2sd" do
      expect(subject.name).to be(:vcvtss2sd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vcvtss2sd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vcvtss2sd" do
      expect(subject.name).to eq(:vcvtss2sd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vcvtss2sd_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem32" do
      ...
      let(:bin_file_name) { "vcvtss2sd_xmm{k}{z}_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vcvtss2sd_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem32" do
      ...
      let(:bin_file_name) { "vcvtss2sd_xmm_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vcvtss2sd_xmm{k}{z}_xmm_xmm_{sae}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vcvtss2sd_xmm_xmm_xmm_{sae}.bin" }
