...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vcvtps2uqq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VCVTPS2UQQ do
  include Helpers::X86_64::Operands

  let(:operands) { [xmm_k_z(0), mem64_mem32_bcst(1)] }

  subject { described_class.new(*operands) }

  describe "#initialize" do
    it "must set #name to :vcvtps2uqq" do
      expect(subject.name).to be(:vcvtps2uqq)
    end
    ...
    context "when given operands of types ymm{k}{z}, mem128/mem32bcst" do
      let(:operands) { [ymm_k_z(0), mem128_mem32_bcst(1)] }
    ...
    context "when given operands of types zmm{k}{z}, mem256/mem32bcst" do
      let(:operands) { [zmm_k_z(0), mem256_mem32_bcst(1)] }
    ...
    context "when given operands of types xmm, mem64/mem32bcst" do
      let(:operands) { [xmm(0), mem64_mem32_bcst(1)] }
    ...
    context "when given operands of types ymm, mem128/mem32bcst" do
      let(:operands) { [ymm(0), mem128_mem32_bcst(1)] }
    ...
    context "when given operands of types zmm, mem256/mem32bcst" do
      let(:operands) { [zmm(0), mem256_mem32_bcst(1)] }
    ...
    context "when given operands of types zmm{k}{z}, ymm, {er}" do
      ...
      it 'must set #form to [:"zmm{k}{z}", :ymm, :"{er}"]' do
        expect(subject.form).to eq([:"zmm{k}{z}", :ymm, :"{er}"])
      end
    end
    ...
    context "when given operands of types zmm, ymm, {er}" do
      ...
      it 'must set #form to [:zmm, :ymm, :"{er}"]' do
        expect(subject.form).to eq([:zmm, :ymm, :"{er}"])
      end
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vcvtps2uqq #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vcvtps2uqq" do
      expect(subject.name).to eq(:vcvtps2uqq)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, mem64/mem32bcst" do
      let(:operands) { [xmm_k_z(0), mem64_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtps2uqq_xmm{k}{z}_mem64mem32bcst.bin" }
    ...
    context "when #operands contains operands of types ymm{k}{z}, mem128/mem32bcst" do
      let(:operands) { [ymm_k_z(0), mem128_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtps2uqq_ymm{k}{z}_mem128mem32bcst.bin" }
    ...
    context "when #operands contains operands of types zmm{k}{z}, mem256/mem32bcst" do
      let(:operands) { [zmm_k_z(0), mem256_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtps2uqq_zmm{k}{z}_mem256mem32bcst.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm" do
      ...
      let(:bin_file_name) { "vcvtps2uqq_xmm{k}{z}_xmm.bin" }
    ...
    context "when #operands contains operands of types ymm{k}{z}, xmm" do
      ...
      let(:bin_file_name) { "vcvtps2uqq_ymm{k}{z}_xmm.bin" }
    ...
    context "when #operands contains operands of types zmm{k}{z}, ymm" do
      ...
      let(:bin_file_name) { "vcvtps2uqq_zmm{k}{z}_ymm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem64/mem32bcst" do
      let(:operands) { [xmm(0), mem64_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtps2uqq_xmm_mem64mem32bcst.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "vcvtps2uqq_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types ymm, mem128/mem32bcst" do
      let(:operands) { [ymm(0), mem128_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtps2uqq_ymm_mem128mem32bcst.bin" }
    ...
    context "when #operands contains operands of types ymm, xmm" do
      ...
      let(:bin_file_name) { "vcvtps2uqq_ymm_xmm.bin" }
    ...
    context "when #operands contains operands of types zmm, mem256/mem32bcst" do
      let(:operands) { [zmm(0), mem256_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtps2uqq_zmm_mem256mem32bcst.bin" }
    ...
    context "when #operands contains operands of types zmm, ymm" do
      ...
      let(:bin_file_name) { "vcvtps2uqq_zmm_ymm.bin" }
    ...
    context "when #operands contains operands of types zmm{k}{z}, ymm, {er}" do
      let(:operands) { [zmm_k_z(0), ymm(1), er(2)] }

      let(:bin_file_name) { "vcvtps2uqq_zmm{k}{z}_ymm_{er}.bin" }
    ...
    context "when #operands contains operands of types zmm, ymm, {er}" do
      let(:operands) { [zmm(0), ymm(1), er(2)] }

      let(:bin_file_name) { "vcvtps2uqq_zmm_ymm_{er}.bin" }
