...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vrcpsh'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VRCPSH do
  ...
  describe "#initialize" do
    it "must set #name to :vrcpsh" do
      expect(subject.name).to be(:vrcpsh)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vrcpsh #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vrcpsh" do
      expect(subject.name).to eq(:vrcpsh)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vrcpsh_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem16" do
      ...
      let(:bin_file_name) { "vrcpsh_xmm{k}{z}_xmm_mem16.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vrcpsh_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem16" do
      ...
      let(:bin_file_name) { "vrcpsh_xmm_xmm_mem16.bin" }
