...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/movdqu'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::MOVDQU do
  ...
  describe "#initialize" do
    it "must set #name to :movdqu" do
      expect(subject.name).to be(:movdqu)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: movdqu #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :movdqu" do
      expect(subject.name).to eq(:movdqu)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "movdqu_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "movdqu_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types mem128, xmm" do
      ...
      let(:bin_file_name) { "movdqu_mem128_xmm.bin" }
