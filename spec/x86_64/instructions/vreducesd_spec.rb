...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vreducesd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VREDUCESD do
  ...
  describe "#initialize" do
    it "must set #name to :vreducesd" do
      expect(subject.name).to be(:vreducesd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vreducesd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vreducesd" do
      expect(subject.name).to eq(:vreducesd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vreducesd_xmm{k}{z}_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem64, imm8" do
      ...
      let(:bin_file_name) { "vreducesd_xmm{k}{z}_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vreducesd_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64, imm8" do
      ...
      let(:bin_file_name) { "vreducesd_xmm_xmm_mem64_imm8.bin" }
