...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vroundps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VROUNDPS do
  ...
  describe "#initialize" do
    it "must set #name to :vroundps" do
      expect(subject.name).to be(:vroundps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vroundps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vroundps" do
      expect(subject.name).to eq(:vroundps)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vroundps_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vroundps_xmm_mem128_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, imm8" do
      ...
      let(:bin_file_name) { "vroundps_ymm_ymm_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, mem256, imm8" do
      ...
      let(:bin_file_name) { "vroundps_ymm_mem256_imm8.bin" }
