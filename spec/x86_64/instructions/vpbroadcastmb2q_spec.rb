...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpbroadcastmb2q'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPBROADCASTMB2Q do
  ...
  describe "#initialize" do
    it "must set #name to :vpbroadcastmb2q" do
      expect(subject.name).to be(:vpbroadcastmb2q)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpbroadcastmb2q #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpbroadcastmb2q" do
      expect(subject.name).to eq(:vpbroadcastmb2q)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, k" do
      ...
      let(:bin_file_name) { "vpbroadcastmb2q_xmm_k.bin" }
    ...
    context "when #operands contains operands of types ymm, k" do
      ...
      let(:bin_file_name) { "vpbroadcastmb2q_ymm_k.bin" }
    ...
    context "when #operands contains operands of types zmm, k" do
      ...
      let(:bin_file_name) { "vpbroadcastmb2q_zmm_k.bin" }
