...
describe Ronin::ASM::X86_64::Instructions::TILELOADD do
  ...
  describe "#initialize" do
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: tileloadd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :tileloadd" do
      expect(subject.name).to eq(:tileloadd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types tmm, sibmem" do
      let(:operands) { [tmm(0), sibmem(1)] }

      let(:bin_file_name) { "tileloadd_tmm_sibmem.bin" }
