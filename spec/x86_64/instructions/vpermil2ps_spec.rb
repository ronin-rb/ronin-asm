...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpermil2ps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPERMIL2PS do
  ...
  describe "#initialize" do
    it "must set #name to :vpermil2ps" do
      expect(subject.name).to be(:vpermil2ps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpermil2ps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpermil2ps" do
      expect(subject.name).to eq(:vpermil2ps)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, xmm, imm8" do
      let(:operands) { [xmm(0), xmm(1), xmm(2), xmm(3), imm8(4)] }

      let(:bin_file_name) { "vpermil2ps_xmm_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, mem128, imm8" do
      let(:operands) { [xmm(0), xmm(1), xmm(2), mem128(3), imm8(4)] }

      let(:bin_file_name) { "vpermil2ps_xmm_xmm_xmm_mem128_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128, xmm, imm8" do
      let(:operands) { [xmm(0), xmm(1), mem128(2), xmm(3), imm8(4)] }

      let(:bin_file_name) { "vpermil2ps_xmm_xmm_mem128_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, ymm, imm8" do
      let(:operands) { [ymm(0), ymm(1), ymm(2), ymm(3), imm8(4)] }

      let(:bin_file_name) { "vpermil2ps_ymm_ymm_ymm_ymm_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, mem256, imm8" do
      let(:operands) { [ymm(0), ymm(1), ymm(2), mem256(3), imm8(4)] }

      let(:bin_file_name) { "vpermil2ps_ymm_ymm_ymm_mem256_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256, ymm, imm8" do
      let(:operands) { [ymm(0), ymm(1), mem256(2), ymm(3), imm8(4)] }

      let(:bin_file_name) { "vpermil2ps_ymm_ymm_mem256_ymm_imm8.bin" }
