...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfmaddss'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFMADDSS do
  ...
  describe "#initialize" do
    it "must set #name to :vfmaddss" do
      expect(subject.name).to be(:vfmaddss)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfmaddss #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfmaddss" do
      expect(subject.name).to eq(:vfmaddss)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfmaddss_xmm_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, mem32" do
      ...
      let(:bin_file_name) { "vfmaddss_xmm_xmm_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem32, xmm" do
      ...
      let(:bin_file_name) { "vfmaddss_xmm_xmm_mem32_xmm.bin" }
