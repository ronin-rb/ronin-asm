...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfmadd231sd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFMADD231SD do
  ...
  describe "#initialize" do
    it "must set #name to :vfmadd231sd" do
      expect(subject.name).to be(:vfmadd231sd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfmadd231sd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfmadd231sd" do
      expect(subject.name).to eq(:vfmadd231sd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfmadd231sd_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem64" do
      ...
      let(:bin_file_name) { "vfmadd231sd_xmm{k}{z}_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfmadd231sd_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64" do
      ...
      let(:bin_file_name) { "vfmadd231sd_xmm_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vfmadd231sd_xmm{k}{z}_xmm_xmm_{er}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vfmadd231sd_xmm_xmm_xmm_{er}.bin" }
