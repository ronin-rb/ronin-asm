...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vmovntdq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VMOVNTDQ do
  ...
  describe "#initialize" do
    it "must set #name to :vmovntdq" do
      expect(subject.name).to be(:vmovntdq)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vmovntdq #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vmovntdq" do
      expect(subject.name).to eq(:vmovntdq)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mem128, xmm" do
      ...
      let(:bin_file_name) { "vmovntdq_mem128_xmm.bin" }
    ...
    context "when #operands contains operands of types mem256, ymm" do
      ...
      let(:bin_file_name) { "vmovntdq_mem256_ymm.bin" }
    ...
    context "when #operands contains operands of types mem512, zmm" do
      ...
      let(:bin_file_name) { "vmovntdq_mem512_zmm.bin" }
