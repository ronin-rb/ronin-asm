...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vroundss'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VROUNDSS do
  ...
  describe "#initialize" do
    it "must set #name to :vroundss" do
      expect(subject.name).to be(:vroundss)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vroundss #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vroundss" do
      expect(subject.name).to eq(:vroundss)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vroundss_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem32, imm8" do
      ...
      let(:bin_file_name) { "vroundss_xmm_xmm_mem32_imm8.bin" }
