...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vsm4key4'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VSM4KEY4 do
  ...
  describe "#initialize" do
    it "must set #name to :vsm4key4" do
      expect(subject.name).to be(:vsm4key4)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vsm4key4 #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vsm4key4" do
      expect(subject.name).to eq(:vsm4key4)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vsm4key4_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vsm4key4_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm" do
      ...
      let(:bin_file_name) { "vsm4key4_ymm_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256" do
      ...
      let(:bin_file_name) { "vsm4key4_ymm_ymm_mem256.bin" }
