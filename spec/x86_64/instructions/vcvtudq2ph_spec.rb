...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vcvtudq2ph'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VCVTUDQ2PH do
  ...
  describe "#initialize" do
    it "must set #name to :vcvtudq2ph" do
      expect(subject.name).to be(:vcvtudq2ph)
    end
    ...
    context "when given operands of types xmm{k}{z}, mem256/mem32bcst" do
      let(:operands) { [xmm_k_z(0), mem256_mem32_bcst(1)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:"xmm{k}{z}", :"mem256/mem32bcst"]' do
        expect(subject.form).to eq([:"xmm{k}{z}", :"mem256/mem32bcst"])
      end
    end

    context "when given operands of types ymm{k}{z}, mem512/mem32bcst" do
      let(:operands) { [ymm_k_z(0), mem512_mem32_bcst(1)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:"ymm{k}{z}", :"mem512/mem32bcst"]' do
        expect(subject.form).to eq([:"ymm{k}{z}", :"mem512/mem32bcst"])
      end
    end
    ...
    context "when given operands of types xmm, mem256/mem32bcst" do
      let(:operands) { [xmm(0), mem256_mem32_bcst(1)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:xmm, :"mem256/mem32bcst"]' do
        expect(subject.form).to eq([:xmm, :"mem256/mem32bcst"])
      end
    end
    ...
    context "when given operands of types ymm, mem512/mem32bcst" do
      let(:operands) { [ymm(0), mem512_mem32_bcst(1)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it 'must set #form to [:ymm, :"mem512/mem32bcst"]' do
        expect(subject.form).to eq([:ymm, :"mem512/mem32bcst"])
      end
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vcvtudq2ph #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vcvtudq2ph" do
      expect(subject.name).to eq(:vcvtudq2ph)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, mem128/mem32bcst" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_xmm{k}{z}_mem128mem32bcst.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, mem256/mem32bcst" do
      let(:operands) { [xmm_k_z(0), mem256_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtudq2ph_xmm{k}{z}_mem256mem32bcst.bin" }
    ...
    context "when #operands contains operands of types ymm{k}{z}, mem512/mem32bcst" do
      let(:operands) { [ymm_k_z(0), mem512_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtudq2ph_ymm{k}{z}_mem512mem32bcst.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_xmm{k}{z}_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, ymm" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_xmm{k}{z}_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm{k}{z}, zmm" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_ymm{k}{z}_zmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128/mem32bcst" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_xmm_mem128mem32bcst.bin" }
    ...
    context "when #operands contains operands of types xmm, mem256/mem32bcst" do
      let(:operands) { [xmm(0), mem256_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtudq2ph_xmm_mem256mem32bcst.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, ymm" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_xmm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, mem512/mem32bcst" do
      let(:operands) { [ymm(0), mem512_mem32_bcst(1)] }

      let(:bin_file_name) { "vcvtudq2ph_ymm_mem512mem32bcst.bin" }
    ...
    context "when #operands contains operands of types ymm, zmm" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_ymm_zmm.bin" }
    ...
    context "when #operands contains operands of types ymm{k}{z}, zmm, {er}" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_ymm{k}{z}_zmm_{er}.bin" }
    ...
    context "when #operands contains operands of types ymm, zmm, {er}" do
      ...
      let(:bin_file_name) { "vcvtudq2ph_ymm_zmm_{er}.bin" }
