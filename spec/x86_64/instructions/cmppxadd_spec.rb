...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/cmppxadd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::CMPPXADD do
  ...
  describe "#initialize" do
    it "must set #name to :cmppxadd" do
      expect(subject.name).to be(:cmppxadd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: cmppxadd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :cmppxadd" do
      expect(subject.name).to eq(:cmppxadd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mem32, reg32, reg32" do
      ...
      let(:bin_file_name) { "cmppxadd_mem32_reg32_reg32.bin" }
    ...
    context "when #operands contains operands of types mem64, reg64, reg64" do
      ...
      let(:bin_file_name) { "cmppxadd_mem64_reg64_reg64.bin" }
