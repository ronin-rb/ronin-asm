...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpshad'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPSHAD do
  ...
  describe "#initialize" do
    it "must set #name to :vpshad" do
      expect(subject.name).to be(:vpshad)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpshad #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpshad" do
      expect(subject.name).to eq(:vpshad)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vpshad_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vpshad_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, xmm" do
      ...
      let(:bin_file_name) { "vpshad_xmm_mem128_xmm.bin" }
