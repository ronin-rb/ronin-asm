...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vmovq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VMOVQ do
  ...
  describe "#initialize" do
    it "must set #name to :vmovq" do
      expect(subject.name).to be(:vmovq)
    end
  ...
  describe "#name" do
    it "must return :vmovq" do
      expect(subject.name).to eq(:vmovq)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types reg64, xmm" do
      ...
      let(:bin_file_name) { "vmovq_reg64_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, reg64" do
      ...
      let(:bin_file_name) { "vmovq_xmm_reg64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "vmovq_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem64" do
      ...
      let(:bin_file_name) { "vmovq_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types mem64, xmm" do
      ...
      let(:bin_file_name) { "vmovq_mem64_xmm.bin" }
