...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vmaxss'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VMAXSS do
  ...
  describe "#initialize" do
    it "must set #name to :vmaxss" do
      expect(subject.name).to be(:vmaxss)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vmaxss #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vmaxss" do
      expect(subject.name).to eq(:vmaxss)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vmaxss_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem32" do
      ...
      let(:bin_file_name) { "vmaxss_xmm{k}{z}_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vmaxss_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem32" do
      ...
      let(:bin_file_name) { "vmaxss_xmm_xmm_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vmaxss_xmm{k}{z}_xmm_xmm_{sae}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vmaxss_xmm_xmm_xmm_{sae}.bin" }
