...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfmsubaddps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFMSUBADDPS do
  ...
  describe "#initialize" do
    it "must set #name to :vfmsubaddps" do
      expect(subject.name).to be(:vfmsubaddps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfmsubaddps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfmsubaddps" do
      expect(subject.name).to eq(:vfmsubaddps)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfmsubaddps_xmm_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, mem128" do
      let(:operands) { [xmm(0), xmm(1), xmm(2), mem128(3)] }

      let(:bin_file_name) { "vfmsubaddps_xmm_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128, xmm" do
      ...
      let(:bin_file_name) { "vfmsubaddps_xmm_xmm_mem128_xmm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, ymm" do
      let(:operands) { [ymm(0), ymm(1), ymm(2), ymm(3)] }

      let(:bin_file_name) { "vfmsubaddps_ymm_ymm_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, mem256" do
      let(:operands) { [ymm(0), ymm(1), ymm(2), mem256(3)] }

      let(:bin_file_name) { "vfmsubaddps_ymm_ymm_ymm_mem256.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256, ymm" do
      let(:operands) { [ymm(0), ymm(1), mem256(2), ymm(3)] }

      let(:bin_file_name) { "vfmsubaddps_ymm_ymm_mem256_ymm.bin" }
