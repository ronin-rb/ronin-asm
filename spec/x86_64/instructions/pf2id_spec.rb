...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/pf2id'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::PF2ID do
  ...
  describe "#initialize" do
    it "must set #name to :pf2id" do
      expect(subject.name).to be(:pf2id)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: pf2id #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :pf2id" do
      expect(subject.name).to eq(:pf2id)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mmx, mmx" do
      ...
      let(:bin_file_name) { "pf2id_mmx_mmx.bin" }
    ...
    context "when #operands contains operands of types mmx, mem64" do
      ...
      let(:bin_file_name) { "pf2id_mmx_mem64.bin" }
