...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/cvtpi2pd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::CVTPI2PD do
  ...
  describe "#initialize" do
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: cvtpi2pd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :cvtpi2pd" do
      expect(subject.name).to eq(:cvtpi2pd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, mmx" do
      ...
      let(:bin_file_name) { "cvtpi2pd_xmm_mmx.bin" }
    ...
    context "when #operands contains operands of types xmm, mem64" do
      ...
      let(:bin_file_name) { "cvtpi2pd_xmm_mem64.bin" }
