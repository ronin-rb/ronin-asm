...
describe Ronin::ASM::X86_64::Instructions::KMOVQ do
  ...
  describe "#initialize" do
    ...
    context "when given operands of types k, reg64" do
      ...
      it "must set #form to [:k, :reg64]" do
        expect(subject.form).to eq([:k, :reg64])
      end
    end
    ...
    context "when given operands of types reg64, k" do
      ...
      it "must set #form to [:reg64, :k]" do
        expect(subject.form).to eq([:reg64, :k])
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types k, k" do
      ...
      let(:bin_file_name) { "kmovq_k_k.bin" }
    ...
    context "when #operands contains operands of types k, reg64" do
      let(:operands) { [k(0), reg64(1)] }

      let(:bin_file_name) { "kmovq_k_reg64.bin" }
    ...
    context "when #operands contains operands of types k, mem64" do
      let(:operands) { [k(0), mem64(1)] }

      let(:bin_file_name) { "kmovq_k_mem64.bin" }
    ...
    context "when #operands contains operands of types reg64, k" do
      let(:operands) { [reg64(0), k(1)] }

      let(:bin_file_name) { "kmovq_reg64_k.bin" }
    ...
    context "when #operands contains operands of types mem64, k" do
      let(:operands) { [mem64(0), k(1)] }

      let(:bin_file_name) { "kmovq_mem64_k.bin" }
