...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vreducesh'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VREDUCESH do
  ...
  describe "#initialize" do
    it "must set #name to :vreducesh" do
      expect(subject.name).to be(:vreducesh)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vreducesh #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vreducesh" do
      expect(subject.name).to eq(:vreducesh)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vreducesh_xmm{k}{z}_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem16, imm8" do
      ...
      let(:bin_file_name) { "vreducesh_xmm{k}{z}_xmm_mem16_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vreducesh_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem16, imm8" do
      ...
      let(:bin_file_name) { "vreducesh_xmm_xmm_mem16_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vreducesh_xmm{k}{z}_xmm_xmm_{sae}_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vreducesh_xmm_xmm_xmm_{sae}_imm8.bin" }
