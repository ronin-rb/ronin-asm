...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/movlps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::MOVLPS do
  ...
  describe "#initialize" do
    it "must set #name to :movlps" do
      expect(subject.name).to be(:movlps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: movlps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :movlps" do
      expect(subject.name).to eq(:movlps)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, mem64" do
      ...
      let(:bin_file_name) { "movlps_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types mem64, xmm" do
      ...
      let(:bin_file_name) { "movlps_mem64_xmm.bin" }
