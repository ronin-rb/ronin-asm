...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/movq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::MOVQ do
  include Helpers::X86_64::Operands

  let(:operands) { [reg64(0), mmx(1)] }
  ...
  describe "#initialize" do
    ...
    context "when given operands of types reg64, mmx" do
      let(:operands) { [reg64(0), mmx(1)] }
      ...
      it "must set #form to [:reg64, :mmx]" do
        expect(subject.form).to eq([:reg64, :mmx])
      end
    end
    ...
    context "when given operands of types mmx, reg64" do
      let(:operands) { [mmx(0), reg64(1)] }

      it "must set #operands" do
        expect(subject.operands).to eq(operands)
      end

      it "must set #form to [:mmx, :reg64]" do
        expect(subject.form).to eq([:mmx, :reg64])
      end
    end
  ...
  describe "#name" do
    it "must return :movq" do
      expect(subject.name).to eq(:movq)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types reg64, mmx" do
      let(:operands) { [reg64(0), mmx(1)] }

      let(:bin_file_name) { "movq_reg64_mmx.bin" }
    ...
    context "when #operands contains operands of types reg64, xmm" do
      ...
      let(:bin_file_name) { "movq_reg64_xmm.bin" }
    ...
    context "when #operands contains operands of types mmx, reg64" do
      let(:operands) { [mmx(0), reg64(1)] }

      let(:bin_file_name) { "movq_mmx_reg64.bin" }
    ...
    context "when #operands contains operands of types mmx, mmx" do
      ...
      let(:bin_file_name) { "movq_mmx_mmx.bin" }
    ...
    context "when #operands contains operands of types mmx, mem64" do
      ...
      let(:bin_file_name) { "movq_mmx_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, reg64" do
      ...
      let(:bin_file_name) { "movq_xmm_reg64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "movq_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem64" do
      ...
      let(:bin_file_name) { "movq_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types mem64, mmx" do
      let(:operands) { [mem64(0), mmx(1)] }

      let(:bin_file_name) { "movq_mem64_mmx.bin" }
    ...
    context "when #operands contains operands of types mem64, xmm" do
      ...
      let(:bin_file_name) { "movq_mem64_xmm.bin" }
