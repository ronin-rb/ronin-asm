...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vprotb'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPROTB do
  ...
  describe "#initialize" do
    it "must set #name to :vprotb" do
      expect(subject.name).to be(:vprotb)
    end
  ...
  describe "#name" do
    it "must return :vprotb" do
      expect(subject.name).to eq(:vprotb)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vprotb_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vprotb_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vprotb_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vprotb_xmm_mem128_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, xmm" do
      ...
      let(:bin_file_name) { "vprotb_xmm_mem128_xmm.bin" }
