...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/punpcklwd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::PUNPCKLWD do
  ...
  describe "#initialize" do
    it "must set #name to :punpcklwd" do
      expect(subject.name).to be(:punpcklwd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: punpcklwd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :punpcklwd" do
      expect(subject.name).to eq(:punpcklwd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mmx, mmx" do
      ...
      let(:bin_file_name) { "punpcklwd_mmx_mmx.bin" }
    ...
    context "when #operands contains operands of types mmx, mem32" do
      ...
      let(:bin_file_name) { "punpcklwd_mmx_mem32.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "punpcklwd_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "punpcklwd_xmm_mem128.bin" }
