...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpblendd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPBLENDD do
  ...
  describe "#initialize" do
    it "must set #name to :vpblendd" do
      expect(subject.name).to be(:vpblendd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpblendd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpblendd" do
      expect(subject.name).to eq(:vpblendd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vpblendd_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vpblendd_xmm_xmm_mem128_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, imm8" do
      ...
      let(:bin_file_name) { "vpblendd_ymm_ymm_ymm_imm8.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256, imm8" do
      ...
      let(:bin_file_name) { "vpblendd_ymm_ymm_mem256_imm8.bin" }
