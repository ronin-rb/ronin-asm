...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vmaxph'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VMAXPH do
  ...
  describe "#initialize" do
    it "must set #name to :vmaxph" do
      expect(subject.name).to be(:vmaxph)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vmaxph #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vmaxph" do
      expect(subject.name).to eq(:vmaxph)
    end
  end
