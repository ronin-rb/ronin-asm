...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vmovhps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VMOVHPS do
  ...
  describe "#initialize" do
    it "must set #name to :vmovhps" do
      expect(subject.name).to be(:vmovhps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vmovhps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vmovhps" do
      expect(subject.name).to eq(:vmovhps)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mem64, xmm" do
      ...
      let(:bin_file_name) { "vmovhps_mem64_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64" do
      ...
      let(:bin_file_name) { "vmovhps_xmm_xmm_mem64.bin" }
