...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpscatterqq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPSCATTERQQ do
  ...
  describe "#initialize" do
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpscatterqq #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpscatterqq" do
      expect(subject.name).to eq(:vpscatterqq)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types vm64x{k}, xmm" do
      ...
      let(:bin_file_name) { "vpscatterqq_vm64x{k}_xmm.bin" }
    ...
    context "when #operands contains operands of types vm64y{k}, ymm" do
      ...
      let(:bin_file_name) { "vpscatterqq_vm64y{k}_ymm.bin" }
    ...
    context "when #operands contains operands of types vm64z{k}, zmm" do
      ...
      let(:bin_file_name) { "vpscatterqq_vm64z{k}_zmm.bin" }
