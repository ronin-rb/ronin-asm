...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/movups'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::MOVUPS do
  ...
  describe "#initialize" do
    it "must set #name to :movups" do
      expect(subject.name).to be(:movups)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: movups #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :movups" do
      expect(subject.name).to eq(:movups)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "movups_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "movups_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types mem128, xmm" do
      ...
      let(:bin_file_name) { "movups_mem128_xmm.bin" }
