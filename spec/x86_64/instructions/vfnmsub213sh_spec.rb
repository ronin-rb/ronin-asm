...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfnmsub213sh'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFNMSUB213SH do
  ...
  describe "#initialize" do
    it "must set #name to :vfnmsub213sh" do
      expect(subject.name).to be(:vfnmsub213sh)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfnmsub213sh #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfnmsub213sh" do
      expect(subject.name).to eq(:vfnmsub213sh)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfnmsub213sh_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem16" do
      ...
      let(:bin_file_name) { "vfnmsub213sh_xmm{k}{z}_xmm_mem16.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfnmsub213sh_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem16" do
      ...
      let(:bin_file_name) { "vfnmsub213sh_xmm_xmm_mem16.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vfnmsub213sh_xmm{k}{z}_xmm_xmm_{er}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vfnmsub213sh_xmm_xmm_xmm_{er}.bin" }
