...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpcmpistrm'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPCMPISTRM do
  ...
  describe "#initialize" do
    it "must set #name to :vpcmpistrm" do
      expect(subject.name).to be(:vpcmpistrm)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpcmpistrm #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpcmpistrm" do
      expect(subject.name).to eq(:vpcmpistrm)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vpcmpistrm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vpcmpistrm_xmm_mem128_imm8.bin" }
