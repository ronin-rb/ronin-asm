...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfixupimmps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFIXUPIMMPS do
  ...
  describe "#initialize" do
    it "must set #name to :vfixupimmps" do
      expect(subject.name).to be(:vfixupimmps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfixupimmps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfixupimmps" do
      expect(subject.name).to eq(:vfixupimmps)
    end
  end
