...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfmsubaddpd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFMSUBADDPD do
  ...
  describe "#initialize" do
    it "must set #name to :vfmsubaddpd" do
      expect(subject.name).to be(:vfmsubaddpd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfmsubaddpd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfmsubaddpd" do
      expect(subject.name).to eq(:vfmsubaddpd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfmsubaddpd_xmm_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vfmsubaddpd_xmm_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128, xmm" do
      ...
      let(:bin_file_name) { "vfmsubaddpd_xmm_xmm_mem128_xmm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, ymm" do
      ...
      let(:bin_file_name) { "vfmsubaddpd_ymm_ymm_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, ymm, mem256" do
      ...
      let(:bin_file_name) { "vfmsubaddpd_ymm_ymm_ymm_mem256.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm, mem256, ymm" do
      ...
      let(:bin_file_name) { "vfmsubaddpd_ymm_ymm_mem256_ymm.bin" }
