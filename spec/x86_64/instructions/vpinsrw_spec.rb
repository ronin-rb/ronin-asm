...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpinsrw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPINSRW do
  ...
  describe "#initialize" do
    it "must set #name to :vpinsrw" do
      expect(subject.name).to be(:vpinsrw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpinsrw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpinsrw" do
      expect(subject.name).to eq(:vpinsrw)
    end
  end
