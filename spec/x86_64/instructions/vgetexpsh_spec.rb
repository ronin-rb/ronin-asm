...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vgetexpsh'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VGETEXPSH do
  ...
  describe "#initialize" do
    it "must set #name to :vgetexpsh" do
      expect(subject.name).to be(:vgetexpsh)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vgetexpsh #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vgetexpsh" do
      expect(subject.name).to eq(:vgetexpsh)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vgetexpsh_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem16" do
      ...
      let(:bin_file_name) { "vgetexpsh_xmm{k}{z}_xmm_mem16.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vgetexpsh_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem16" do
      ...
      let(:bin_file_name) { "vgetexpsh_xmm_xmm_mem16.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vgetexpsh_xmm{k}{z}_xmm_xmm_{sae}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vgetexpsh_xmm_xmm_xmm_{sae}.bin" }
