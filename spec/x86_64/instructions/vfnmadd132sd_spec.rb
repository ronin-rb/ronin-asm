...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfnmadd132sd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFNMADD132SD do
  ...
  describe "#initialize" do
    it "must set #name to :vfnmadd132sd" do
      expect(subject.name).to be(:vfnmadd132sd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfnmadd132sd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfnmadd132sd" do
      expect(subject.name).to eq(:vfnmadd132sd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfnmadd132sd_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem64" do
      ...
      let(:bin_file_name) { "vfnmadd132sd_xmm{k}{z}_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfnmadd132sd_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64" do
      ...
      let(:bin_file_name) { "vfnmadd132sd_xmm_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vfnmadd132sd_xmm{k}{z}_xmm_xmm_{er}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vfnmadd132sd_xmm_xmm_xmm_{er}.bin" }
