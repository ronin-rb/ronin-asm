...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/pswapd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::PSWAPD do
  ...
  describe "#initialize" do
    it "must set #name to :pswapd" do
      expect(subject.name).to be(:pswapd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: pswapd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :pswapd" do
      expect(subject.name).to eq(:pswapd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mmx, mmx" do
      ...
      let(:bin_file_name) { "pswapd_mmx_mmx.bin" }
    ...
    context "when #operands contains operands of types mmx, mem64" do
      ...
      let(:bin_file_name) { "pswapd_mmx_mem64.bin" }
