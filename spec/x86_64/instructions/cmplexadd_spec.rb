...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/cmplexadd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::CMPLEXADD do
  ...
  describe "#initialize" do
    it "must set #name to :cmplexadd" do
      expect(subject.name).to be(:cmplexadd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: cmplexadd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :cmplexadd" do
      expect(subject.name).to eq(:cmplexadd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mem32, reg32, reg32" do
      ...
      let(:bin_file_name) { "cmplexadd_mem32_reg32_reg32.bin" }
    ...
    context "when #operands contains operands of types mem64, reg64, reg64" do
      ...
      let(:bin_file_name) { "cmplexadd_mem64_reg64_reg64.bin" }
