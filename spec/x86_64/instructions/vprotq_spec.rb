...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vprotq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPROTQ do
  ...
  describe "#initialize" do
    it "must set #name to :vprotq" do
      expect(subject.name).to be(:vprotq)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vprotq #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vprotq" do
      expect(subject.name).to eq(:vprotq)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vprotq_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vprotq_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vprotq_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vprotq_xmm_mem128_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, xmm" do
      let(:operands) { [xmm(0), mem128(1), xmm(2)] }

      let(:bin_file_name) { "vprotq_xmm_mem128_xmm.bin" }
