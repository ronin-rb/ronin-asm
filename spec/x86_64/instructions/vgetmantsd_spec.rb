...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vgetmantsd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VGETMANTSD do
  ...
  describe "#initialize" do
    it "must set #name to :vgetmantsd" do
      expect(subject.name).to be(:vgetmantsd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vgetmantsd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vgetmantsd" do
      expect(subject.name).to eq(:vgetmantsd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsd_xmm{k}{z}_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem64, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsd_xmm{k}{z}_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsd_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsd_xmm_xmm_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsd_xmm{k}{z}_xmm_xmm_{sae}_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsd_xmm_xmm_xmm_{sae}_imm8.bin" }
