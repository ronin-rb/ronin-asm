...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/movsd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::MOVSD do
  ...
  describe "#initialize" do
    it "must set #name to :movsd" do
      expect(subject.name).to be(:movsd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: movsd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :movsd" do
      expect(subject.name).to eq(:movsd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "movsd_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem64" do
      ...
      let(:bin_file_name) { "movsd_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types mem64, xmm" do
      ...
      let(:bin_file_name) { "movsd_mem64_xmm.bin" }
