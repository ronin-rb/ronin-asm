...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/sha1rnds4'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::SHA1RNDS4 do
  ...
  describe "#initialize" do
    it "must set #name to :sha1rnds4" do
      expect(subject.name).to be(:sha1rnds4)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: sha1rnds4 #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "sha1rnds4_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "sha1rnds4_xmm_mem128_imm8.bin" }
