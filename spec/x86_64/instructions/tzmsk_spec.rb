...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/tzmsk'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::TZMSK do
  ...
  describe "#initialize" do
    it "must set #name to :tzmsk" do
      expect(subject.name).to be(:tzmsk)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: tzmsk #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :tzmsk" do
      expect(subject.name).to eq(:tzmsk)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types reg32, reg32" do
      ...
      let(:bin_file_name) { "tzmsk_reg32_reg32.bin" }
    ...
    context "when #operands contains operands of types reg32, mem32" do
      ...
      let(:bin_file_name) { "tzmsk_reg32_mem32.bin" }
    ...
    context "when #operands contains operands of types reg64, reg64" do
      ...
      let(:bin_file_name) { "tzmsk_reg64_reg64.bin" }
    ...
    context "when #operands contains operands of types reg64, mem64" do
      ...
      let(:bin_file_name) { "tzmsk_reg64_mem64.bin" }
