...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpcomuw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPCOMUW do
  ...
  describe "#initialize" do
    it "must set #name to :vpcomuw" do
      expect(subject.name).to be(:vpcomuw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpcomuw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpcomuw" do
      expect(subject.name).to eq(:vpcomuw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vpcomuw_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "vpcomuw_xmm_xmm_mem128_imm8.bin" }
