...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpscatterdq'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPSCATTERDQ do
  ...
  describe "#initialize" do
    it "must set #name to :vpscatterdq" do
      expect(subject.name).to be(:vpscatterdq)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpscatterdq #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types vm32x{k}, xmm" do
      ...
      let(:bin_file_name) { "vpscatterdq_vm32x{k}_xmm.bin" }
    ...
    context "when #operands contains operands of types vm32x{k}, ymm" do
      ...
      let(:bin_file_name) { "vpscatterdq_vm32x{k}_ymm.bin" }
    ...
    context "when #operands contains operands of types vm32y{k}, zmm" do
      ...
      let(:bin_file_name) { "vpscatterdq_vm32y{k}_zmm.bin" }
