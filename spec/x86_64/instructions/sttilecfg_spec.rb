...
describe Ronin::ASM::X86_64::Instructions::STTILECFG do
  ...
  describe "#initialize" do
    it "must set #name to :sttilecfg" do
      expect(subject.name).to be(:sttilecfg)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: sttilecfg #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :sttilecfg" do
      expect(subject.name).to eq(:sttilecfg)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mem512" do
      ...
      let(:bin_file_name) { "sttilecfg_mem512.bin" }
