...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpshlw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPSHLW do
  ...
  describe "#initialize" do
    it "must set #name to :vpshlw" do
      expect(subject.name).to be(:vpshlw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpshlw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpshlw" do
      expect(subject.name).to eq(:vpshlw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vpshlw_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem128" do
      ...
      let(:bin_file_name) { "vpshlw_xmm_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, xmm" do
      ...
      let(:bin_file_name) { "vpshlw_xmm_mem128_xmm.bin" }
