...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vmulsh'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VMULSH do
  ...
  describe "#initialize" do
    it "must set #name to :vmulsh" do
      expect(subject.name).to be(:vmulsh)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vmulsh #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vmulsh_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem16" do
      ...
      let(:bin_file_name) { "vmulsh_xmm{k}{z}_xmm_mem16.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vmulsh_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem16" do
      ...
      let(:bin_file_name) { "vmulsh_xmm_xmm_mem16.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vmulsh_xmm{k}{z}_xmm_xmm_{er}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {er}" do
      ...
      let(:bin_file_name) { "vmulsh_xmm_xmm_xmm_{er}.bin" }
