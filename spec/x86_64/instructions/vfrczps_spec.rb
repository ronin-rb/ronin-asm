...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfrczps'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFRCZPS do
  ...
  describe "#initialize" do
    it "must set #name to :vfrczps" do
      expect(subject.name).to be(:vfrczps)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfrczps #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vfrczps" do
      expect(subject.name).to eq(:vfrczps)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "vfrczps_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "vfrczps_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm" do
      ...
      let(:bin_file_name) { "vfrczps_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, mem256" do
      ...
      let(:bin_file_name) { "vfrczps_ymm_mem256.bin" }
