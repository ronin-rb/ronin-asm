...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/psllw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::PSLLW do
  ...
  describe "#initialize" do
    it "must set #name to :psllw" do
      expect(subject.name).to be(:psllw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: psllw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :psllw" do
      expect(subject.name).to eq(:psllw)
    end
  end
