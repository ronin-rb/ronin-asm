...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vrcp28sd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VRCP28SD do
  ...
  describe "#initialize" do
    it "must set #name to :vrcp28sd" do
      expect(subject.name).to be(:vrcp28sd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vrcp28sd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vrcp28sd" do
      expect(subject.name).to eq(:vrcp28sd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm" do
      ...
      let(:bin_file_name) { "vrcp28sd_xmm{k}{z}_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem64" do
      ...
      let(:bin_file_name) { "vrcp28sd_xmm{k}{z}_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vrcp28sd_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64" do
      ...
      let(:bin_file_name) { "vrcp28sd_xmm_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vrcp28sd_xmm{k}{z}_xmm_xmm_{sae}.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}" do
      ...
      let(:bin_file_name) { "vrcp28sd_xmm_xmm_xmm_{sae}.bin" }
