...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vtestpd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VTESTPD do
  ...
  describe "#initialize" do
    it "must set #name to :vtestpd" do
      expect(subject.name).to be(:vtestpd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vtestpd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "vtestpd_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "vtestpd_xmm_mem128.bin" }
    ...
    context "when #operands contains operands of types ymm, ymm" do
      ...
      let(:bin_file_name) { "vtestpd_ymm_ymm.bin" }
    ...
    context "when #operands contains operands of types ymm, mem256" do
      ...
      let(:bin_file_name) { "vtestpd_ymm_mem256.bin" }
