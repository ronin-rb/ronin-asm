...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vgetmantsh'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VGETMANTSH do
  ...
  describe "#initialize" do
    it "must set #name to :vgetmantsh" do
      expect(subject.name).to be(:vgetmantsh)
    end
    ...
    context "when given operands of types xmm{k}{z}, xmm, mem16, imm8" do
      let(:operands) { [xmm_k_z(0), xmm(1), mem16(2), imm8(3)] }
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vgetmantsh #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vgetmantsh" do
      expect(subject.name).to eq(:vgetmantsh)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsh_xmm{k}{z}_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, mem16, imm8" do
      let(:operands) { [xmm_k_z(0), xmm(1), mem16(2), imm8(3)] }

      let(:bin_file_name) { "vgetmantsh_xmm{k}{z}_xmm_mem16_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsh_xmm_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem16, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsh_xmm_xmm_mem16_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm{k}{z}, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsh_xmm{k}{z}_xmm_xmm_{sae}_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, {sae}, imm8" do
      ...
      let(:bin_file_name) { "vgetmantsh_xmm_xmm_xmm_{sae}_imm8.bin" }
