...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vpextrw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VPEXTRW do
  ...
  describe "#initialize" do
    it "must set #name to :vpextrw" do
      expect(subject.name).to be(:vpextrw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vpextrw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :vpextrw" do
      expect(subject.name).to eq(:vpextrw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types reg32, xmm, imm8" do
      ...
      let(:bin_file_name) { "vpextrw_reg32_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types mem16, xmm, imm8" do
      ...
      let(:bin_file_name) { "vpextrw_mem16_xmm_imm8.bin" }
