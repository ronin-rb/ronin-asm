...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/palignr'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::PALIGNR do
  ...
  describe "#initialize" do
    it "must set #name to :palignr" do
      expect(subject.name).to be(:palignr)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: palignr #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :palignr" do
      expect(subject.name).to eq(:palignr)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mmx, mmx, imm8" do
      let(:operands) { [mmx(0), mmx(1), imm8(2)] }

      let(:bin_file_name) { "palignr_mmx_mmx_imm8.bin" }
    ...
    context "when #operands contains operands of types mmx, mem64, imm8" do
      let(:operands) { [mmx(0), mem64(1), imm8(2)] }

      let(:bin_file_name) { "palignr_mmx_mem64_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "palignr_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "palignr_xmm_mem128_imm8.bin" }
