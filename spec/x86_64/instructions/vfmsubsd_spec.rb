...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/vfmsubsd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::VFMSUBSD do
  ...
  describe "#initialize" do
    it "must set #name to :vfmsubsd" do
      expect(subject.name).to be(:vfmsubsd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: vfmsubsd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, xmm" do
      ...
      let(:bin_file_name) { "vfmsubsd_xmm_xmm_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, xmm, mem64" do
      ...
      let(:bin_file_name) { "vfmsubsd_xmm_xmm_xmm_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm, mem64, xmm" do
      ...
      let(:bin_file_name) { "vfmsubsd_xmm_xmm_mem64_xmm.bin" }
