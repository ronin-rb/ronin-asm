...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/mulx'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::MULX do
  ...
  describe "#initialize" do
    it "must set #name to :mulx" do
      expect(subject.name).to be(:mulx)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: mulx #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :mulx" do
      expect(subject.name).to eq(:mulx)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types reg32, reg32, reg32" do
      ...
      let(:bin_file_name) { "mulx_reg32_reg32_reg32.bin" }
    ...
    context "when #operands contains operands of types reg32, reg32, mem32" do
      let(:operands) { [reg32(0), reg32(1), mem32(2)] }

      let(:bin_file_name) { "mulx_reg32_reg32_mem32.bin" }
    ...
    context "when #operands contains operands of types reg64, reg64, reg64" do
      ...
      let(:bin_file_name) { "mulx_reg64_reg64_reg64.bin" }
    ...
    context "when #operands contains operands of types reg64, reg64, mem64" do
      let(:operands) { [reg64(0), reg64(1), mem64(2)] }

      let(:bin_file_name) { "mulx_reg64_reg64_mem64.bin" }
