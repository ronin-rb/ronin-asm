...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/cmppd'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::CMPPD do
  ...
  describe "#initialize" do
    it "must set #name to :cmppd" do
      expect(subject.name).to be(:cmppd)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: cmppd #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :cmppd" do
      expect(subject.name).to eq(:cmppd)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types xmm, xmm, imm8" do
      ...
      let(:bin_file_name) { "cmppd_xmm_xmm_imm8.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128, imm8" do
      ...
      let(:bin_file_name) { "cmppd_xmm_mem128_imm8.bin" }
