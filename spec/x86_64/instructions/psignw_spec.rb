...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/psignw'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::PSIGNW do
  ...
  describe "#initialize" do
    it "must set #name to :psignw" do
      expect(subject.name).to be(:psignw)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: psignw #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :psignw" do
      expect(subject.name).to eq(:psignw)
    end
  end
  ...
  describe "#encode", :compatibility do
    ...
    context "when #operands contains operands of types mmx, mmx" do
      ...
      let(:bin_file_name) { "psignw_mmx_mmx.bin" }
    ...
    context "when #operands contains operands of types mmx, mem64" do
      ...
      let(:bin_file_name) { "psignw_mmx_mem64.bin" }
    ...
    context "when #operands contains operands of types xmm, xmm" do
      ...
      let(:bin_file_name) { "psignw_xmm_xmm.bin" }
    ...
    context "when #operands contains operands of types xmm, mem128" do
      ...
      let(:bin_file_name) { "psignw_xmm_mem128.bin" }
