...
require 'spec_helper'
require 'ronin/asm/x86_64/instructions/jp'

require_relative '../helpers/operands'

describe Ronin::ASM::X86_64::Instructions::JP do
  ...
  describe "#initialize" do
    it "must set #name to :jp" do
      expect(subject.name).to be(:jp)
    end
    ...
    context "when given operands that do not match any of the instruction's forms" do
      ...
      it do
        ...
        }.to raise_error(ArgumentError,"incompatible operands given for instruction: jp #{operands.map(&:type).join(', ')}")
      end
    end
  ...
  describe "#name" do
    it "must return :jp" do
      expect(subject.name).to eq(:jp)
    end
  end
