  ...
  module X86
    ...
    class InstructionsFile < CodeGen::InstructionsFile
      ...
      def instruction_examples(instruction)
        instruction.forms.map do |instruction_form|
      ...
      def example_immediate(operand_type,index)
        ...
        when :imm16     then format('word(0x%.4x)',index+1)
        when :imm32     then format('dword(0x%.8x)',index+1)
      ...
      # Mapping of operand sizes and size specifiers.
      SIZE_METHODS = {
        1  => 'byte',
        2  => 'word',
        4  => 'dword',
      ...
      # @param [String, nil] size
      # @return [String]
      #
      def memory_operand(base: , index: nil, scale: nil, size: nil)
        operand = if    index && scale then "[#{base}+#{index}*#{scale}]"
                  elsif index          then "[#{base}+#{index}]"
                  else                      "[#{base}]"
                  end

        if size
          size_method = SIZE_METHODS.fetch(size)
      ...
      def example_memory(operand_type,index)
        ...
        size = MEMORY_TYPE_SIZES.fetch(operand_type) do
          raise(ArgumentError,"unknown memory type: #{operand_type.inspect}")
        end

        memory_operand(base: base, size: size)
      end
      ...
      def example_vector_memory(operand_type,index)
        ...
        memory_operand(base: base, index: index, scale: size, size: size)
      end
