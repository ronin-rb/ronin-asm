    ...
    module X86_64
      module Instructions
        ...
        class VFRCZPD < Instruction
          ...
          #
          # Initializes the `vfrczpd` instruction.
          #
          ...
          def initialize(*operands,**kwargs)
            super(:vfrczpd,*operands,**kwargs)
          end

          #
          # Encodes the `vfrczpd` instruction.
          #
          ...
          def encode(encoder)
            if @operands.length == 2 && @operands[0].type == :xmm && @operands[1].type == :xmm
              ...
              encoder.write_vex(type: :xop, w: 0, l: 1, m_mmmm: 0b01001, pp: 0b00, r: @operands[0], b: @operands[1], vvvv: 0) +
              encoder.write_opcode(0x81) +
              encoder.write_modrm(3,@operands[0],@operands[1])
            elsif @operands.length == 2 && @operands[0].type == :ymm && @operands[1].type == :m256
              encoder.write_vex(type: :xop, w: 0, l: 1, m_mmmm: 0b01001, pp: 0b00, r: @operands[0], x: @operands[1], b: @operands[1], vvvv: 0) +
              encoder.write_opcode(0x81) +
              encoder.write_modrm(@operands[1],@operands[0],@operands[1])
            else
