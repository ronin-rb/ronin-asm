    ...
    module X86_64
      module Instructions
        ...
        class VPGATHERDD < Instruction
          ...
          #
          # Initializes the `vpgatherdd` instruction.
          #
          ...
          def initialize(*operands,**kwargs)
            super(:vpgatherdd,*operands,**kwargs)
          end

          #
          # Encodes the `vpgatherdd` instruction.
          #
          ...
          def encode(encoder)
            if @operands.length == 2 && @operands[0].type == :"xmm{k}" && @operands[1].type == :vm32x
              encoder.write_evex(mmm: 0b010, pp: 0b01, w: 0, ll: 0, vvvv: 0, v: "#<data CodeGen::X86::ISA::OperandIndex index=1>", rr: @operands[0], _B: @operands[1], x: @operands[1], b: 0, aaa: @operands[0], z: 0, disp8xN: 4) +
              encoder.write_opcode(0x90) +
              encoder.write_modrm(@operands[1],@operands[0],@operands[1])
            elsif @operands.length == 2 && @operands[0].type == :"ymm{k}" && @operands[1].type == :vm32y
              encoder.write_evex(mmm: 0b010, pp: 0b01, w: 0, ll: 1, vvvv: 0, v: "#<data CodeGen::X86::ISA::OperandIndex index=1>", rr: @operands[0], _B: @operands[1], x: @operands[1], b: 0, aaa: @operands[0], z: 0, disp8xN: 4) +
              encoder.write_opcode(0x90) +
              encoder.write_modrm(@operands[1],@operands[0],@operands[1])
            elsif @operands.length == 2 && @operands[0].type == :"zmm{k}" && @operands[1].type == :vm32z
              encoder.write_evex(mmm: 0b010, pp: 0b01, w: 0, ll: 2, vvvv: 0, v: "#<data CodeGen::X86::ISA::OperandIndex index=1>", rr: @operands[0], _B: @operands[1], x: @operands[1], b: 0, aaa: @operands[0], z: 0, disp8xN: 4) +
              encoder.write_opcode(0x90) +
              encoder.write_modrm(@operands[1],@operands[0],@operands[1])
