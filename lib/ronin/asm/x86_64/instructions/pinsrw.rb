    ...
    module X86_64
      module Instructions
        ...
        class PINSRW < Instruction
          ...
          #
          def encode(encoder)
            if @operands.length == 3 && @operands[0].type_of?(:mmx) && @operands[1].type_of?(:reg32) && @operands[2].type_of?(:imm8)
            ...
            elsif @operands.length == 3 && @operands[0].type_of?(:mmx) && @operands[1].type_of?(:mem16) && @operands[2].type_of?(:imm8)
              encoder.write_rex(mandatory: false, w: 0, r: @operands[0], x: @operands[1], b: @operands[1]) +
              encoder.write_opcode(0x0f) +
              encoder.write_opcode(0xc4) +
              encoder.write_modrm(@operands[1],@operands[0],@operands[1]) +
              encoder.write_immediate(@operands[2],1)
            elsif @operands.length == 3 && @operands[0].type_of?(:xmm) && @operands[1].type_of?(:reg32) && @operands[2].type_of?(:imm8)
