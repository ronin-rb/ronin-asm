        ...
        class VADDSD < Instruction
          ...
          def encode(encoder)
            if @operands.length == 3 && @operands[0].type_of?(:"xmm{k}{z}") && @operands[1].type_of?(:xmm) && @operands[2].type_of?(:xmm)
              ...
              encoder.write_evex(mmm: 0b001, pp: 0b11, ll: @operands[3], w: 1, vvvv: @operands[1], v: @operands[1], rr: @operands[0], _B: @operands[2], x: @operands[2], b: 1, aaa: @operands[0], z: @operands[0]) +
              encoder.write_opcode(0x58) +
              encoder.write_modrm(0b11,@operands[0],@operands[2])
            elsif @operands.length == 4 && @operands[0].type_of?(:xmm) && @operands[1].type_of?(:xmm) && @operands[2].type_of?(:xmm) && @operands[3].type_of?(:"{er}")
              encoder.write_evex(mmm: 0b001, pp: 0b11, ll: @operands[3], w: 1, vvvv: @operands[1], v: @operands[1], rr: @operands[0], _B: @operands[2], x: @operands[2], b: 1, aaa: 0, z: 0) +
              encoder.write_opcode(0x58) +
              encoder.write_modrm(0b11,@operands[0],@operands[2])
            else
