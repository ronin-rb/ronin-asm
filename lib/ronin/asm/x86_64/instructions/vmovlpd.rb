        ...
        class VMOVLPD < Instruction
          ...
          def encode(encoder)
            if @operands.length == 2 && @operands[0].type == :mem64 && @operands[1].type == :xmm
              ...
              encoder.write_evex(mmm: 0b001, pp: 0b01, w: 1, ll: 0, vvvv: @operands[1], v: "#<data CodeGen::X86::ISA::OperandIndex index=1>", rr: @operands[0], _B: @operands[2], x: @operands[2], b: 0, aaa: 0, z: 0, disp8xN: 8) +
              encoder.write_opcode(0x12) +
              encoder.write_modrm(@operands[2],@operands[0],@operands[2])
            else
