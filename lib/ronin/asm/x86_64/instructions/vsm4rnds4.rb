  ...
  module ASM
    module X86_64
      module Instructions
        ...
        #
        class VSM4RNDS4 < Instruction

          #
          # Initializes the `vsm4rnds4` instruction.
          #
          ...
          def initialize(*operands,**kwargs)
            super(:vsm4rnds4,*operands,**kwargs)
          end

          #
          # Encodes the `vsm4rnds4` instruction.
          #
          ...
          def encode(encoder)
            if @operands.length == 3 && @operands[0].type_of?(:xmm) && @operands[1].type_of?(:xmm) && @operands[2].type_of?(:xmm)
              ...
              encoder.write_vex(type: :vex, w: 0, l: 1, m_mmmm: 0b00010, pp: 0b11, r: @operands[0], b: @operands[2], vvvv: @operands[1]) +
              encoder.write_opcode(0xda) +
              encoder.write_modrm(0b11,@operands[0],@operands[2])
            elsif @operands.length == 3 && @operands[0].type_of?(:ymm) && @operands[1].type_of?(:ymm) && @operands[2].type_of?(:mem256)
              encoder.write_vex(type: :vex, w: 0, l: 1, m_mmmm: 0b00010, pp: 0b11, r: @operands[0], x: @operands[2], b: @operands[2], vvvv: @operands[1]) +
              encoder.write_opcode(0xda) +
              encoder.write_modrm(@operands[2],@operands[0],@operands[2])
            else
