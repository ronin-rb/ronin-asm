        ...
        class VDIVSD < Instruction
          ...
          def encode(encoder)
            if @operands.length == 3 && @operands[0].type == :"xmm{k}{z}" && @operands[1].type == :xmm && @operands[2].type == :xmm
              ...
              encoder.write_evex(0b001,0b11,1,@operands[3],@operands[1],0,0b00,0,0,1,@operands[0],@operands[0],nil) +
              encoder.write_opcode(0x5e) +
              encoder.write_modrm(3,@operands[0],@operands[2],@operands)
            elsif @operands.length == 4 && @operands[0].type == :xmm && @operands[1].type == :xmm && @operands[2].type == :xmm && @operands[3].type == :"{er}"
              encoder.write_evex(0b001,0b11,1,@operands[3],@operands[1],0,0b00,0,0,1,0,0,nil) +
              encoder.write_opcode(0x5e) +
              encoder.write_modrm(3,@operands[0],@operands[2],@operands)
            else
