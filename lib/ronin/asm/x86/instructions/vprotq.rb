    ...
    module X86
      module Instructions
        ...
        class VPROTQ < Instruction
          ...
          #
          # Initializes the `vprotq` instruction.
          #
          ...
          def initialize(*operands,**kwargs)
            super(:vprotq,*operands,**kwargs)
          end

          #
          # Encodes the `vprotq` instruction.
          #
          ...
          def encode(encoder)
            if @operands.length == 3 && @operands[0].type == :xmm && @operands[1].type == :xmm && @operands[2].type == :imm8
              encoder.write_vex(type: :xop, w: 0, l: 0, m_mmmm: 0b01000, pp: 0b00, r: 0, x: 0, b: 0, vvvv: 0) +
              ...
              encoder.write_vex(type: :xop, w: 0, l: 0, m_mmmm: 0b01001, pp: 0b00, r: 0, x: 0, b: 0, vvvv: @operands[2]) +
              encoder.write_opcode(0x93) +
              encoder.write_modrm(3,@operands[0],@operands[1])
            elsif @operands.length == 3 && @operands[0].type == :xmm && @operands[1].type == :xmm && @operands[2].type == :m128
              encoder.write_vex(type: :xop, w: 1, l: 0, m_mmmm: 0b01001, pp: 0b00, r: 0, x: 0, b: 0, vvvv: @operands[1]) +
              encoder.write_opcode(0x93) +
