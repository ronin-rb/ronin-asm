        ...
        class VUCOMISD < Instruction
          ...
          def encode(encoder)
            if @operands.length == 2 && @operands[0].type == :xmm && @operands[1].type == :xmm
              encoder.write_vex(:vex,nil,nil,0b00001,0b01,0,0,0,0) +
              ...
              encoder.write_evex(0b001,0b01,1,0,0,0,0b00,0,0,0,0,0,8) +
              encoder.write_opcode(0x2e) +
              encoder.write_modrm(@operands[1],@operands[0],@operands[1],@operands)
            elsif @operands.length == 3 && @operands[0].type == :xmm && @operands[1].type == :xmm && @operands[2].type == :"{sae}"
              encoder.write_evex(0b001,0b01,1,nil,0,0,0b00,0,0,1,0,0,nil) +
              encoder.write_opcode(0x2e) +
              encoder.write_modrm(3,@operands[0],@operands[1],@operands)
            else
