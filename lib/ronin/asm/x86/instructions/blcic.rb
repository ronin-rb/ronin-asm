    ...
    module X86
      module Instructions
        ...
        class BLCIC < Instruction
          ...
          #
          # Initializes the `blcic` instruction.
          #
          ...
          def initialize(*operands,**kwargs)
            super(:blcic,*operands,**kwargs)
          end

          #
          # Encodes the `blcic` instruction.
          #
          ...
          def encode(encoder)
            if @operands.length == 2 && @operands[0].type == :reg32 && @operands[1].type == :reg32
              ...
              encoder.write_modrm(3,5,@operands[1],@operands)
            elsif @operands.length == 2 && @operands[0].type == :reg32 && @operands[1].type == :mem32
              encoder.write_vex(:xop,0,0,0b01001,0b00,0,0,0,@operands[0]) +
              encoder.write_opcode(0x01) +
              encoder.write_modrm(@operands[1],5,@operands[1],@operands)
            else
