        ...
        class VRCP28SD < Instruction
          ...
          def encode(encoder)
            if @operands.length == 3 && @operands[0].type == :"xmm{k}{z}" && @operands[1].type == :xmm && @operands[2].type == :xmm
              ...
              encoder.write_evex(0b010,0b01,1,0,@operands[1],0,0b00,0,0,0,@operands[0],@operands[0],8) +
              encoder.write_opcode(0xcb) +
              ...
              encoder.write_evex(0b010,0b01,1,0,@operands[1],0,0b00,0,0,0,0,0,8) +
              encoder.write_opcode(0xcb) +
              encoder.write_modrm(@operands[2],@operands[0],@operands[2],@operands)
            elsif @operands.length == 4 && @operands[0].type == :"xmm{k}{z}" && @operands[1].type == :xmm && @operands[2].type == :xmm && @operands[3].type == :"{sae}"
              encoder.write_evex(0b010,0b01,1,nil,@operands[1],0,0b00,0,0,1,@operands[0],@operands[0],nil) +
              encoder.write_opcode(0xcb) +
              encoder.write_modrm(3,@operands[0],@operands[2],@operands)
            elsif @operands.length == 4 && @operands[0].type == :xmm && @operands[1].type == :xmm && @operands[2].type == :xmm && @operands[3].type == :"{sae}"
              encoder.write_evex(0b010,0b01,1,nil,@operands[1],0,0b00,0,0,1,0,0,nil) +
              encoder.write_opcode(0xcb) +
              encoder.write_modrm(3,@operands[0],@operands[2],@operands)
            else
